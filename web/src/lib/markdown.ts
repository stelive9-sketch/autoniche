...
function toWindows1252Bytes(value: string): Uint8Array {
...
export function getSortedArticlesData(): ArticleData[] {
  ...
  const allArticlesData = fileNames.filter(f => f.endsWith('.md')).map((fileName) => {
    ...
    const title = repairMojibake((matterResult.data as { title?: string }).title || '');
    const description = repairMojibake((matterResult.data as { description?: string }).description || '');
...
export async function getArticleData(slug: string): Promise<ArticleData | null> {
  ...
  const fileContents = fs.readFileSync(fullPath, 'utf8');
  const matterResult = matter(fileContents);
  const normalizedContent = repairMojibake(matterResult.content);
  const normalizedTitle = repairMojibake((matterResult.data as { title?: string }).title || '');
  const normalizedDescription = repairMojibake((matterResult.data as { description?: string }).description || '');
  const affiliateLinks = extractAmazonLinks(normalizedContent);
  const comparisonModels = extractComparisonModels(normalizedContent, affiliateLinks);
